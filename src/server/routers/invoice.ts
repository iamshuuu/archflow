...
type TimeEntryForInvoicing = Prisma.TimeEntryGetPayload<{
    include: {
        user: { select: { name: true; billRate: true } };
        project: { select: { id: true; name: true } };
        phase: { select: { name: true; assignments: { select: { userId: true; billRate: true } } } };
    };
}>;
...
export const invoiceRouter = router({
    ...
    updateStatus: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            const user = await ctx.db.user.findUnique({ where: { email: ctx.session!.user!.email! } });
            if (!user) throw new TRPCError({ code: "UNAUTHORIZED", message: "User not found" });
            const invoice = await ctx.db.invoice.findFirst({ where: { id: input.id, orgId: user.orgId }, select: { id: true } });
            if (!invoice) throw new TRPCError({ code: "FORBIDDEN", message: "Invoice not found or inaccessible" });
    ...
    unbilledWork: protectedProcedure.query(async ({ ctx }) => {
        ...
        const projectMap: Record<string, { projectId: string; projectName: string; entries: Array<TimeEntryForInvoicing & { rate: number }>; totalHours: number; totalValue: number }> = {};
    ...
    arAging: protectedProcedure.query(async ({ ctx }) => {
        ...
        const today = new Date();
        const buckets = { current: [] as InvoiceWithPayments[], bucket30: [] as InvoiceWithPayments[], bucket60: [] as InvoiceWithPayments[], bucket90: [] as InvoiceWithPayments[], bucket90plus: [] as InvoiceWithPayments[] };
    ...
    recordPayment: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            const totalPaid = payment.invoice.payments.reduce((s: number, p) => s + p.amount, 0);
    ...
    generateFromTime: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            entries.forEach((e) => {
                ...
                byUser[name].entries.push(`${e.date}: ${e.notes || "Work"} (${e.hours}h)`);
            });
    ...
    batchCreate: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            const user = await ctx.db.user.findUnique({ where: { email: ctx.session!.user!.email! } });
            if (!user) throw new Error("User not found");
            const results: Prisma.InvoiceGetPayload<{ include: { client: true; project: true; lineItems: true } }>[] = [];
    ...
    generatePdf: protectedProcedure
        ...
        .query(async ({ ctx, input }) => {
            ...
            const user = await ctx.db.user.findUnique({ where: { email: ctx.session!.user!.email! } });
            if (!user) throw new Error("User not found");
            if (!invoice || invoice.orgId !== user.orgId) throw new Error("Invoice not found");
            const invoiceData = invoice as InvoiceForPdf;

            // Get template
            const template = (await ctx.db.invoiceTemplate.findFirst({
                where: { orgId: invoice.orgId, isDefault: true },
            })) || {
                companyName: invoiceData.org?.name || "ArchFlow Studio",
                address: "", phone: "", email: "",
                headerText: "", footerText: "Payment is due within 30 days.",
                accentColor: "#B07A4A", logo: "",
            } as TemplateLike;

            const orgLocale = invoiceData.org?.locale || "en-US";
            const orgCurrency = invoiceData.org?.currency || "USD";
            ...
            const lineItemsHtml = invoiceData.lineItems.map((li) => `
                <tr>
...
<html><head><meta charset="UTF-8"><title>Invoice ${invoiceData.number}</title>
<style>
body { font-family: 'Segoe UI', system-ui, sans-serif; margin:0; padding:40px; color:#333; background:#fff; }
.header { display:flex; justify-content:space-between; margin-bottom:40px; }
.company { font-size:20px; font-weight:700; color:${template.accentColor}; }
...
th { background:${template.accentColor}10; color:${template.accentColor}; font-size:10px; text-transform:uppercase; letter-spacing:0.06em; padding:10px 12px; text-align:left; }
th:nth-child(2) { text-align:center; } th:nth-child(3), th:nth-child(4) { text-align:right; }
.total-row { font-size:18px; font-weight:700; color:${template.accentColor}; text-align:right; padding:16px 12px; border-top:2px solid ${template.accentColor}; }
...
<div class="header">
    <div>
        ${template.logo ? `<img src="${template.logo}" style="height:40px;margin-bottom:8px" />` : ""}
        <div class="company">${template.companyName}</div>
        <p style="font-size:11px;color:#999;margin:4px 0">${template.address}</p>
        ${template.phone ? `<p style="font-size:11px;color:#999;margin:2px 0">${template.phone}</p>` : ""}
        ${template.email ? `<p style="font-size:11px;color:#999;margin:2px 0">${template.email}</p>` : ""}
    </div>
    <div>
...
${template.headerText ? `<p style="font-size:13px;color:#666;margin-bottom:20px">${template.headerText}</p>` : ""}
<div class="info-grid">
    <div class="info-box">
        <h4>Bill To</h4>
        <p style="font-weight:600">${invoiceData.client?.name || "—"}</p>
    </div>
    <div class="info-box" style="text-align:right">
        <h4>Details</h4>
        <p>Project: ${invoiceData.project?.name || "—"}</p>
        <p>Date: ${invoiceData.date}</p>
        <p>Due: ${invoiceData.dueDate}</p>
        <p>Status: ${invoiceData.status.toUpperCase()}</p>
    </div>
</div>
<table>
...
<div class="total-row">Total: ${formatMoney(invoiceData.amount)}</div>
${invoiceData.notes ? `<div class="notes"><strong>Notes:</strong> ${invoiceData.notes}</div>` : ""}
${invoiceData.paymentUrl ? `<p style="margin-top:16px;font-size:13px">Pay online: <a href="${invoiceData.paymentUrl}" style="color:${template.accentColor}">${invoiceData.paymentUrl}</a></p>` : ""}
<div class="footer">${template.footerText}</div>
</body></html>`;

            return { html, number: invoiceData.number };
        }),
